...
## Corpus Check
- 69 files · ~30,610 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 640 nodes · 1228 edges · 33 communities (29 shown, 4 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 38 edges (avg confidence: 0.7)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c37fe529`
...
- map.md
- test_comparar_modelos_global.py
...
1. `pronosticar_seasonal_naive()` - 29 edges
2. `construir_dataset_supervisado()` - 20 edges
3. `pronosticar_futuro_lightgbm_global()` - 20 edges
4. `pronosticar_tsb()` - 18 edges
5. `entrenar_lightgbm_global()` - 17 edges
6. `backtest_walk_forward()` - 16 edges
7. `_ajustar_ets()` - 16 edges
8. `_serie()` - 16 edges
9. `clasificar_demanda()` - 15 edges
10. `combinar_pronosticos()` - 15 edges
...
- `main()` --calls--> `cargar_ventas()`  [EXTRACTED]
  src/forecast/ejecutar_backtest_benchmark.py → src/datos/cargar_datos.py
...
Cohesion: 0.06
Nodes (43): FuncionPronostico, backtest_walk_forward(), DataFrame, Series, Backtest walk-forward (rolling), sin split único ni k-fold — ver spec.md:19…, backtest_y_predicciones_lightgbm_global(), comparar_modelos_con_lightgbm_global(), _iterar_predicciones_por_sku() (+35 more)
...
Cohesion: 0.09
Nodes (32): NamedTuple, Wrapper compartido de fallback — usado por modelo_ets.py y modelo_prophet.py:…, estimar_tendencia(), pronosticar_seasonal_naive(), ndarray, Series, Benchmark Seasonal Naive + drift condicional — piso de comparación del MVP…, Wrapper compatible con el uso anterior (solo bool) — ver `estimar_tendencia` si… (+24 more)
...
### Community 6 - "test_comparar_modelos_global.py"
Cohesion: 0.10
Nodes (22): _ajustar_benchmark(), backtest_lightgbm_global(), Una fila por SKU, con las mismas columnas que produce `comparar_modelos_sku`…, ndarray, Series, El benchmark es el destino del fallback de los demás — nunca cae en fallback él…, DataFrame, Selección del mejor modelo por SKU — criterio decidido en .scratch/motor-… (+14 more)
...
Cohesion: 0.13
Nodes (20): comparar_modelos(), comparar_modelos_sku(), DataFrame, Igual que `comparar_modelos_sku`, para todos los SKUs de `ventas`., Las unidades vendidas/pronosticadas nunca son negativas — un modelo puede…, Una fila por candidato, con sus métricas agregadas del backtest y su tasa de…, _sin_negativos(), _sin_negativos_con_metadata() (+12 more)

### Community 9 - "comparar_modelos.py"
Cohesion: 0.15
Nodes (19): Exception, ajustar_con_fallback(), ndarray, Series, Corre `ajustar(serie, horizonte)`. Si lanza una de `excepciones`, cae a…, Corre el backtest walk-forward de benchmark vs. modelo por SKU y devuelve la…, _ajustar_modelo_prophet(), _ajustar_prophet() (+11 more)
...
Cohesion: 0.15
Nodes (12): evaluar_ensemble(), evaluar_ensemble_por_sku(), DataFrame, ndarray, Una fila por SKU con el candidato "ensemble", mismo esquema que…, Métricas del ensemble evaluadas por walk-forward anidado (ver docstring del…, DataFrame, Tests de ensemble_backtest.py: el candidato "ensemble" compite en… (+4 more)
...
- **Why does `pronosticar_seasonal_naive()` connect `pronosticar_seasonal_naive` to `pronosticar_futuro.py`, `comparar_modelos_global.py`, `pronosticar_directo`, `test_comparar_modelos_global.py`, `comparar_modelos.py`, `_ajustar_xgboost`?**
  _High betweenness centrality (0.080) - this node is a cross-community bridge._
- **Why does `construir_dataset_supervisado()` connect `construir_dataset_supervisado` to `pronosticar_futuro.py`, `comparar_modelos_global.py`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Why does `pronosticar_tsb()` connect `pronosticar_tsb` to `comparar_modelos.py`, `comparar_modelos_global.py`?**
